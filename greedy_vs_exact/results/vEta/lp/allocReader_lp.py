def readSW(cTypes_u, cTypes_l):
    for i in range(len(cTypes_u)):
        for j in range(100):
            fileName = f'/home/jps/allocation_models/greedy_vs_exact/results/vEta/lp/cl{cTypes_u[i]}/inst_{j}.txt'
            ...
            with open(fileName, 'r') as f:
                for line in f:
                    if line.startswith('allocated users: ('):
                        result = line.split(' ')[2:]
                        print(", ".join(result[:-1]).replace(',,', ',')[:-1])
        print(fileName)
        print('-----------')


def main():
    cTypes_u = ['A_mult_small_20', 'D_mult_mixed_20']
    cTypes_l = ['e_10']
